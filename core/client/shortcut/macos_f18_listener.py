...
class MacOSF18Listener:
    ...
    def start(self) -> None:
        ...
        if self._tap is None:
            logger.warning(
                ...
                "回退到 pynput 被动监听（终端内长按 Caps Lock 会出现 ^[[32~ ）。"
            )
    ...
    def _start_fallback(self) -> None:
        """Accessibility 未授权时回退到 pynput 被动监听。"""
        ...
        def on_press(key):
            ...
            with self._lock:
                if self._pressed:
                    return
                self._pressed = True
            logger.info("[f18-listener] F18 down (fallback)")
            self._on_down()
        ...
        def on_release(key):
            ...
            with self._lock:
                if not self._pressed:
                    return
                self._pressed = False
            logger.info("[f18-listener] F18 up (fallback)")
            self._on_up()
        ...
        logger.info("[f18-listener] pynput fallback started")

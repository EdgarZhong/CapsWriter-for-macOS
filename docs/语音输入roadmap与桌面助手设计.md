...
> **修订注记（2026-09-23）**：本节"热词摘出为服务/热词服务化"的口径已废止，以《[macOS 产品化架构定稿](macos-产品化架构定稿.md)》为最高口径——热词**不服务化**，作为 App 托管的 private Python helper（stdin/stdout JSON，无端口）；终局也不再保留 Python headless client，Swift App 自身即客户端 runtime。下文保留原始记录，冲突处以定稿文档为准。

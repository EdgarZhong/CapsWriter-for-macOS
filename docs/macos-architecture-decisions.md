...
> **修订注记（2026-09-23）**：本节"热词摘出为独立热词服务"的口径已被《[macOS 产品化架构定稿](macos-产品化架构定稿.md)》取代——热词**不服务化**，作为 App 托管的 private Python helper（stdin/stdout JSON，不监听端口）；且终局不再保留 Python headless client，Swift App 自身即客户端 runtime。本节其余口径（客户端主导编排、不建统一插件底座、ASR 独立服务、DMG 分发）仍然有效；冲突处以定稿文档为准。

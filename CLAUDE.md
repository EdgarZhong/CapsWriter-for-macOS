...
- 平台基线已收敛：CapsWriter for macOS 只支持 Apple Silicon（arm64）与 macOS 26.0+；Dashboard 构建固定使用 CLT 26.6、Swift/SwiftPM 6.3.x、macOS 26.5 SDK，不再维护旧系统回退。
...
- 2026-09-21 全屏能力收敛：Dashboard 产品决策改为只支持普通窗口，不维护 macOS 全屏布局。窗口配置清理互斥的 full-screen collection flags 后设置 `.fullScreenNone` 与 `.fullScreenDisallowsTiling`；macOS 26 根视图使用官方 `windowFullScreenBehavior(.disabled)` 作为 SwiftUI WindowGroup 的最终约束。全屏入口验证以最新 PID `75228` 为准：AX 树显示 `zoom button` 而非 `full screen button`，`Control-Command-F` 后窗口仍为 `1000×700pt` 普通窗口；`Fn` 键本身因 CUA 不支持而未能直接发送。
  ...
  - 上述扩散原因仍是实现层假设，截图只证明本机当前构建的两个场景；减少透明度、窗口 resize 和完整第五节验收仍未完成，旧系统回退已按新的平台基线停止维护。
...
- 验证：5 组 Swift 状态兼容检查、窗口编译与开发包签名校验通过；实机检查四页导航、真实状态显示、浅色/深色切换及跟随系统回切。全屏 capability 禁用已完成构建和运行时验证；减少透明度仅完成代码接入，未实机验收。其余页面操作、原生听写迁移、完整 DMG 和用户验收未完成。
...
- 未改个人配置、词库或日常服务；开发窗口使用独立 Bundle ID。工具链恢复基线已提交为 `aae1247`，全屏 capability 禁用已提交为 `bc1d342`。

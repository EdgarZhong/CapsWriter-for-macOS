...
class QwenAudioEncoder:
    ...
    def __init__(self, frontend_path: str, backend_path: str, use_dml: bool = True, dml_device_id: int = 0, pad_to: int = 30, verbose: bool = True):
        ...
        # 强制设置执行优先级: DML(1) -> CPU
        # 注意: ort.get_available_providers() 返回的列表顺序不代表优先级，
        # InferenceSession 构造函数中的 providers 参数顺序才决定优先级。
        
        providers = []
        if use_dml:
            # 检查 DmlExecutionProvider 是否可用
            available_providers = ort.get_available_providers()
            if 'DmlExecutionProvider' in available_providers:
                # 显式构造 provider 选项，指定设备 ID
                # 注意：DmlExecutionProvider 的选项值必须是 True/False 字符串，不能是 0/1
                dml_provider = ('DmlExecutionProvider', {
                    'device_id': str(dml_device_id), # 确保是字符串
                    'skip_options': 'False',
                ...
                if self.verbose: print(f"[Warning] DmlExecutionProvider requested but not available. Available: {available_providers}")
        
        # 总是添加 CPU 作为回退
        providers.append('CPUExecutionProvider')
            
        if self.verbose: 
            print(f"--- [Encoder] 加载 Split ONNX 模型 (DML: {self.active_dml}, Device: {dml_device_id}, Pad: {pad_to}s) ---")
        ...
        try:
            ...
            print(f"[Error] Failed to create InferenceSession with DML. Falling back to CPU. Error: {e}")
            self.active_dml = False
